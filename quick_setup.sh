#!/bin/bash
#===============================================================================
# InterPro MCP Quick Setup Script
#===============================================================================
# This script sets up the complete environment for InterPro MCP server.
#
...
echo -e "${BLUE}"
echo "=============================================="
echo "      InterPro MCP Quick Setup Script        "
echo "=============================================="
echo -e "${NC}"
...
if [ "$SKIP_ENV" = true ]; then
    ...
    "${ENV_DIR}/bin/pip" install loguru click pandas numpy tqdm

    info "Installing MCP dependencies..."
    "${ENV_DIR}/bin/pip" install "mcp>=1.1.0" "pydantic>=2.0.0" "pydantic-settings>=2.0.0" "httpx>=0.24.0"

    info "Installing test dependencies..."
    "${ENV_DIR}/bin/pip" install "pytest>=7.0.0" "pytest-asyncio>=0.21.0" "ruff>=0.1.0"

    info "Installing fastmcp..."
    "${ENV_DIR}/bin/pip" install --ignore-installed fastmcp loguru --force-reinstall --no-cache-dir
...
"${ENV_DIR}/bin/python" -c "import fastmcp; import loguru; import httpx; print('Core packages OK')" && success "Core packages verified" || error "Package verification failed"
...
echo "  claude mcp add interpro -- ${ENV_DIR}/bin/python ${SCRIPT_DIR}/src/server.py"
echo ""
